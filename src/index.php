...
// 2. Extrahiere den Suchbegriff aus dem XML-CMD
$searchTerm = '';
if (preg_match('/<application>(.*?)<\/application>/i', $cmd, $matches)) {
    $searchTerm = trim($matches[1]);
}

// Prüfe auf <all></all> oder <all/> verschachtelt
$isAllQuery = (
    empty($searchTerm) ||
...
// Wenn kein <all> vorliegt, Tags entfernen
$cleanSearch = $isAllQuery ? 'all' : trim(strip_tags($searchTerm));

// Prüfe die Art der Anfrage
$isDataSummaryQuery = (stripos($cmd, '<cloud-app-data>') !== false);
$isWildcardSearch    = (!$isAllQuery && (strpos($cleanSearch, '%') !== false || strpos($cleanSearch, '*') !== false));
...
if ($isDataSummaryQuery) {
    ...
    // Speicheranzeigeschonender XML-Output direkt per Echo / Streaming
...
if ($isWildcardSearch || $isAllQuery) {
    if ($isAllQuery) {
        $stmt = $db->query("SELECT xml_content FROM cloud_appids WHERE xml_content IS NOT NULL AND xml_content != ''");
    } else {
        $sqlPattern = str_replace(['%', '*'], '%', $cleanSearch);
        $stmt = $db->prepare("SELECT xml_content FROM cloud_appids WHERE name LIKE :pattern AND xml_content IS NOT NULL AND xml_content != ''");
        $stmt->execute([':pattern' => $sqlPattern]);
    }
} else {
...
// Wenn mehrere XML-Einträge zusammengeführt werden
http_response_code(200);
echo '<?xml version="1.0" encoding="UTF-8"?>' . "\n";
echo '<response status="success">' . "\n";
echo '  <result>' . "\n";
