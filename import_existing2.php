...
// Prepared Statement - JETZT MIT explicit ID
...
foreach ($files as$file) {
    ...
    // Echte Objekt-ID auslesen (prüft <id> unter Wurzel, unter <entry> oder Attribut 'id')
    $id = 0;
    if (isset($xml->id)) {
        $id = (int)$xml->id;
    ...
    $stmt->execute([
        ':id' => $id > 0 ?$id : null, // Falls keine ID im XML existiert, greift NULL (Auto-Increment)
        ':name' => $name,
